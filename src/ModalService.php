          ...
          if (!empty($end_date)) {
            if ($current_date > $end_date) {
              ...
              try {
                ...
                $modal->save();
                \Drupal::logger('cadence')->info('ModalService: Modal @id (@label) automatically disabled - end date (@end_date) has passed.', [
                  '@id' => $modal->id(),
                  '@label' => $modal->label(),
                  '@end_date' => $end_date,
                ]);
                ...
                \Drupal::logger('cadence')->warning('ModalService: Failed to disable expired modal @id: @message', [
                  '@id' => $modal->id(),
      ...
      try {
        ...
        if (!empty($image_data)) {
          try {
              ...
              foreach ($image_data['fids'] as $fid) {
                if ($fid && (int) $fid > 0) {
                  ...
                  if ($file && $file->isPermanent()) {
                    ...
                    \Drupal::logger('cadence')->warning('ModalService: Image file (FID: @fid) not found or not permanent for modal @id.', [
                      '@fid' => $fid,
                      '@id' => $modal->id(),
                    ]);
                  }
            ...
            \Drupal::logger('cadence')->warning('ModalService: Error processing image for modal @id: @message', [
              '@id' => $modal->id(),
        ...
        \Drupal::logger('cadence')->error('Error processing modal @id: @message', [
          '@id' => $modal->id(),
          '@message' => $e->getMessage(),
        ]);
        ...
        try {
          ...
          \Drupal::logger('cadence')->error('Error adding modal @id to array: @message', [
            '@id' => $modal->id(),
