...
class ModalAnalyticsController extends ControllerBase {
  ...
  public function trackEvent(Request $request) {
    ...
    try {
      ...
      \Drupal::logger('cadence')->error('Error tracking analytics event: @message', [
  ...
  protected function isIpExcluded(Request $request) {
    $config = \Drupal::config('cadence.settings');
  ...
  public function analytics(Request $request) {
    ...
    $tabs = [
      'analytics' => [
        ...
        'url' => Url::fromRoute('cadence.modal.analytics', [], ['query' => array_merge($current_query, ['tab' => 'analytics'])]),
      ],
      'charts' => [
        'title' => $this->t('Charts'),
        'url' => Url::fromRoute('cadence.modal.analytics', [], ['query' => array_merge($current_query, ['tab' => 'charts'])]),
      ],
      'settings' => [
        'title' => $this->t('Settings'),
        'url' => Url::fromRoute('cadence.modal.analytics', [], ['query' => array_merge($current_query, ['tab' => 'settings'])]),
      ],
    ];
    ...
    if ($active_tab === 'settings') {
      $build['content'] = \Drupal::formBuilder()->getForm('Drupal\cadence\Form\AnalyticsSettingsForm');
    }
  ...
  protected function analyticsContent(Request $request) {
    try {
      ...
      try {
        ...
        $form_submissions = 0;
        \Drupal::logger('cadence')->warning('Error querying form submissions for modal @modal_id: @message', [
          '@modal_id' => $modal_id,
          '@message' => $e->getMessage(),
        ]);
      }
    ...
    if (!$show_archived) {
      ...
      $build = [
        ...
        '#attached' => [
          'library' => ['cadence/modal.analytics'],
          'drupalSettings' => [
            'modalAnalytics' => [
      ...
      \Drupal::logger('cadence')->error('Error loading analytics: @message', [
        '@message' => $e->getMessage(),
      ]);
      return [
  ...
  protected function chartsContent(Request $request) {
    try {
      ...
      \Drupal::logger('cadence')->error('Error loading charts: @message', [
        '@message' => $e->getMessage(),
      ]);
      return [
  ...
  public function reset(Request $request) {
    try {
      ...
      if (!$connection->schema()->tableExists('modal_analytics')) {
        ...
        return new RedirectResponse(Url::fromRoute('cadence.modal.analytics')->toString());
      }
      ...
      \Drupal::logger('cadence')->notice('Analytics data reset by user @user', [
      ...
      \Drupal::logger('cadence')->error('Failed to reset analytics data: @error', [
        '@error' => $e->getMessage(),
      ]);
    }
