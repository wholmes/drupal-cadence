  ...
  protected function cleanupAnalyticsData($modal_id) {
    try {
      ...
      if ($database->schema()->tableExists('modal_analytics')) {
        ...
        \Drupal::logger('cadence')->info('Deleted @count analytics records for modal @modal_id', [
          '@count' => $deleted,
          '@modal_id' => $modal_id,
        ]);
      }
      ...
      \Drupal::logger('cadence')->error('Failed to clean up analytics data for modal @modal_id: @error', [
        '@modal_id' => $modal_id,
        '@error' => $e->getMessage(),
      ]);
    }
    ...
    try {
      ...
      foreach ($fids_to_cleanup as $fid) {
        ...
        if ($file) {
          ...
          \Drupal::service('file.usage')->delete($file, 'cadence', 'modal', $this->entity->id());
      ...
      if (!empty($image_data['mobile_fid'])) {
        ...
        if ($mobile_file) {
          // Remove file usage tracking.
          \Drupal::service('file.usage')->delete($mobile_file, 'cadence', 'modal', $this->entity->id());
      ...
      \Drupal::logger('cadence')->error('Failed to clean up files for modal @modal_id: @error', [
        '@modal_id' => $this->entity->id(),
        '@error' => $e->getMessage(),
      ]);
    }
