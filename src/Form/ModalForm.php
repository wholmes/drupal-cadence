...
class ModalForm extends EntityForm {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    $form['#attached']['library'][] = 'cadence/admin';
    
    // Attach form persistence library for collapsible panels.
    $form['#attached']['library'][] = 'cadence/modal.form.persistence';
    ...
    $form['content']['image']['carousel']['duration']['#attached']['library'][] = 'cadence/modal.form.persistence';
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    if ($images_removed) {
      ...
      foreach ($existing_fids as $old_fid) {
        ...
        if ($old_file) {
          \Drupal::service('file.usage')->delete($old_file, 'cadence', 'modal', $modal->id());
        }
      }
      $image_fids = [];
    }
    elseif (!empty($form_fids)) {
      ...
      foreach ($form_fids as $fid) {
        if ($fid > 0) {
          ...
          if ($file) {
            ...
            // Track file usage.
            \Drupal::service('file.usage')->add($file, 'cadence', 'modal', $modal->id());
            $image_fids[] = $fid;
          }
        }
      }
      ...
      foreach ($removed_fids as $removed_fid) {
        ...
        if ($removed_file) {
          \Drupal::service('file.usage')->delete($removed_file, 'cadence', 'modal', $modal->id());
        }
      }
    }
    else {
    ...
    $image_fid = !empty($image_fids) ? reset($image_fids) : NULL;
        ...
        if ($old_mobile_fid && $old_mobile_fid !== $mobile_image_fid) {
          ...
          if ($old_mobile_file) {
            \Drupal::service('file.usage')->delete($old_mobile_file, 'cadence', 'modal', $modal->id());
          }
        }
        ...
        if ($mobile_image_fid && $mobile_image_fid > 0) {
          ...
          if ($mobile_file) {
            ...
            // Track file usage.
            \Drupal::service('file.usage')->add($mobile_file, 'cadence', 'modal', $modal->id());
          }
        }
    ...
    try {
      ...
      \Drupal::logger('cadence')->warning('Error clearing cache after modal save: @message', [
  ...
  public function previewModal(array &$form, FormStateInterface $form_state) {
    ...
    $libraries = [
      ...
      $preview_data['styling']['layout'] === 'bottom_sheet' 
        ? 'cadence/modal.bottom_sheet' 
        : 'cadence/modal.centered',
      'cadence/modal.preview',
    ];
    ...
    if (isset($preview_data['styling']['decorative_effect']) && $preview_data['styling']['decorative_effect'] === 'confetti') {
      $libraries[] = 'cadence/modal.confetti';
    }
  ...
  protected function renderPreview(array $modal_data) {
    ...
    if (isset($modal_data['styling']['decorative_effect']) && $modal_data['styling']['decorative_effect'] === 'confetti') {
      $preview_container['#attached']['library'][] = 'cadence/modal.confetti';
    }
